...
### Why `animation-fill-mode: forwards`?

The library waits for your CSS animation to finish before unmounting the element (or before flipping `data-state` back to `open` when a new modal takes over). Without `forwards`, the element snaps back to the values declared on the base selector as soon as the keyframes end — so the final frame of `card-out` is discarded, and you can see a flicker when the next modal opens on top, when the same element is reused, or during the brief window between animation-end and unmount.

Keep `forwards` on **both** states (`[data-state='open']` and `[data-state='closed']`) so each animation settles on its final frame instead of reverting to the initial CSS.
